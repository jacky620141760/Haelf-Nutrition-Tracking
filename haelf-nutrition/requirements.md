...
Haelf Nutrition 是一款個人使用、iOS 優先、繁體中文介面的 Expo React Native 飲食與體重紀錄 App。v1 採本機優先且免帳號，核心資料儲存在裝置端 SQLite；網路僅用於 Open Food Facts（OFF）條碼資料查詢與使用者主動啟動的 AI 食物分析。Web 僅供開發預覽。

本規格聚焦可驗收的 v1 邊界：飲食日記、營養計算、每日目標、條碼、AI 輔助、體重與趨勢、統計、資料生命週期、可及性及效能。v1 不包含跨裝置同步、雲端備份、帳號、社交、TDEE 自動估算、商店上架或 Web 正式支援。
